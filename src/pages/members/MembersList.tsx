...
import type { Member, MemberStatus } from "../../data/fake/people";
...
type ActionProps = { member: Member; onDisable: (m: Member) => void; onEnable: (m: Member) => void };
...
function MemberAction({ member, onDisable, onEnable }: ActionProps) {
  if (member.status === "active") return <Button size="small" type="text" danger onClick={() => onDisable(member)}>Disable</Button>;
  if (member.status === "suspended") return <Button size="small" onClick={() => onEnable(member)}>Enable</Button>;
  return null;
}
  ...
  return (
    ...
    >
      <Table<Member> {...tableProps} rowKey="id">
        ...
        <Table.Column<Member> title="Status" dataIndex="status" render={(s: MemberStatus) => <StatusTag status={s} label={LABELS[s]} />} />
