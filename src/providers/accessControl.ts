 ...
 * signed-in account's page list, read once per session; a resource that
 * is not a page (a table inside one) is never refused here — the page
 * around it already was.
 */
export const accessControl: AccessControlProvider = {
  can: async ({ resource }) => {
    if (!resource || !isPageKey(resource)) return { can: true };
    const me = (await authProvider.getIdentity?.()) as StaffIdentity | null;
